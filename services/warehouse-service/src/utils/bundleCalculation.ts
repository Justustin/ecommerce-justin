...
export function calculateBundlesNeeded(
  ...
  const baseBundle = Math.floor(quantity / bundleSize);

  // For exact multiples of single bundle, no tolerance needed
  if (remainder === 0 && baseBundle === 1) {
    return 1;
  }

  // For all other cases, add tolerance
